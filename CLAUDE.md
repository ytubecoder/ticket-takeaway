...
src/kitchen_feed.py — /kitchen attention-feed data layer (time-bucketing, unread heuristic, flat items list)
src/kitchen_view.py — /kitchen attention-feed renderer (cards, chips, overflow menu, responsive CSS, polling JS)
...
src/static/        — PWA assets served at root scope (manifest.webmanifest, sw.js, icon-{180,192,512}.png, icon.svg)
```
...
**Kitchen view** (`/kitchen`): the user-facing attention feed (distinct from the orchestrator above). A single responsive page — same layout phone → desktop — that surfaces tickets needing attention, grouped by recency (Today / Yesterday / This week / Older), with two filter sets: run-state chips (Needs me / Running / Ready to delegate / Paused / Failed) at the top, and a project checkbox list inside the "⋯" overflow menu. The header carries an always-visible Live/Paused pill. Cards have a thin bucket-tinted left border + section badge + bucket label; no big pictorial tile. Tap a card → `/{pid}/#ticket/{id}` (the kanban auto-opens the detail overlay on that hash). `/kitchen/demo` renders the same view against a stub state so the layout can be reviewed when the live DB is empty (cards are click-inert with an explanatory banner).

**PWA installability:** the dashboard (and every top-level page) is installable as a standalone app. `manifest.webmanifest` and `sw.js` are served at site root (`/`) so the service-worker scope covers all project-scoped routes. The SW is network-first for navigations, NEVER caches `/api/*`, and falls back to a cached shell when offline. Any new top-level page renderer in `serve.py` must include `PWA_HEAD_TAGS` + `gen.build_nav_rail_css()` + `gen.build_nav_rail_html()` + `gen.build_nav_rail_js()` to stay consistent with installability and the left-rail.

**Workflow Bounce** (I-19, extended by migration #20 — endpoint abstraction; plan: `docs/superpowers/plans/2026-05-11-model-endpoint-abstraction.md`): Multi-agent prompt routing system. Users define **agents** (name + system prompt + `endpoint_id`) and **endpoints** (runtime: command + args + capabilities). Many agents share one endpoint; the endpoint binding is editable even on system agents (persona stays locked). Workflows are ordered steps, each with an agent + optional step instructions. Applying a workflow to a ticket bounces its content through the agent sequence. Primary agent (step 1) mediates disagreements.

- **DB tables:** `workflow_agents`, `workflows`, `workflow_runs` (migration 4); `endpoints` (migration 20)
- **API:** `/api/workflow/agents`, `/api/workflow/workflows` (CRUD), `/api/workflow/workflows/{id}/duplicate` (clone any workflow — including disabled system rows — into a user-owned editable copy; available globally and project-scoped), `/api/workflow/workflows/{id}/preview` (returns count + sample tickets currently matching the trigger across all linked projects, used by /workflows live-match badges), `/api/tickets/{id}/workflow/run` (execution), `/api/workflow/runs/{id}` (status/polling), `/api/workflow/runs/active` (active runs across all tickets for kanban indicators); `/api/endpoints` (CRUD for endpoint records)
- **CLI:** `tickets-cli.py agent list/add/update/remove` (accepts `--endpoint-id`), `workflow list/add/add-step/remove-step/remove`, `endpoint list/add/update/remove`
- **UI:** Agents and workflows are managed in a full-page "Workflows & Agents" view (zap icon in nav bar, `body.bounce-open`); includes an Endpoints tab for managing endpoint records. Settings (Appearance, Feedbacks, Managed Files, Project metadata, Scenarios, Danger Zone) live in the right-hand drawer (gear icon). Ticket detail has workflow dropdown + Run button with instant placeholder and polling.
- **Execution:** Background thread per run. Per step, the runner calls `endpoints.build_invocation(endpoint, prompt, session_id=...)` to get the argv. Endpoint `type='cli'` runs locally via subprocess; other types (`anthropic_api`, `openai_api`, `gemini_api`, `ssh_cli`) are schema-reserved but raise `UnsupportedEndpointType` at invocation (phase 1). `--no-session-persistence` flag, 120s timeout (`WORKFLOW_AGENT_TIMEOUT`). Progress conversation entries flushed to DB before each subprocess call. Return code checked; stderr surfaced on failure. Disagreement detection via primary agent evaluation after each step.
- **Compat fallback:** When `workflow_agents.endpoint_id IS NULL`, the runner falls back to the legacy `workflow_agents.command`/`args` columns. Logged once per agent per server boot (`WARN runner: agent=<id> using compat command ...`). Non-NULL `endpoint_id` pointing at a missing or non-cli endpoint is a HARD ERROR with no fallback.
- **System rows (`system=1`) are mostly read-only — definition lives in source.** Editor renders most inputs disabled with a banner pointing at `workflows_seed.py`. Server enforces:
  - **Workflows:** `PUT` with non-`enabled` fields and `DELETE` on system workflows return `403 system_workflow`. To customise: `POST .../duplicate` creates an editable user copy.
  - **Agents:** `DELETE` returns `403 system_agent`. `PUT` accepts ONLY `endpoint_id` changes on system rows (everything else returns `403 system_agent` with `forbidden_fields` in the body). Persona (name, system_prompt) stays locked because the seed re-upserts it; endpoint binding is unlocked because the seed's UPDATE clause omits `endpoint_id` (preserves user choice across re-seeds). To customise persona, edit `workflows_seed.py` and restart serve.py.
  - System and user workflows are otherwise **symmetric**: same dispatcher loop, same ticket pool, same trigger evaluator, same on_success effects. The only execution difference is `ORDER BY system DESC, id ASC` in the dispatcher's workflow query, which gives system rows precedence on identical-trigger overlaps (deterministic tie-breaker, not a privilege gate).
- **Source-of-truth invariant for ship.** Every system agent + workflow that ships must round-trip through `workflows_seed.py` — the live DB is a materialised copy. `src/compare_seed_to_db.py` audits drift (system rows missing from DB, user rows present in DB, system flag mismatches); run before declaring a ship version.
- **Master automation switch:** Every default system workflow that mutates a ticket includes an `automation_mode='auto'` predicate in `trigger_json`. The per-ticket automation toggle (`automation_subjects.automation_mode`) is therefore the master switch — flipping a ticket to manual exempts it from every default rule. User workflows can include or omit that predicate as their use case requires; the dispatcher does NOT impose a separate pool-level filter (used to, removed 2026-05-09).
- **Workflow → project linkage:** Per-project enable state lives in `workflow_projects(workflow_id, project_id, enabled)`, NOT in `workflows.enabled` / `workflows.project_id` (those were the pre-migration-16 model and are stale for system rows where `project_id IS NULL`). The kitchen dispatcher SQL must JOIN `workflow_projects` and filter on `wp.enabled = 1 AND wp.project_id = ?`. The global PUT `/api/workflow/workflows/{id}` mirrors `enabled` into all linked rows.
- **Trigger sentence helper:** `src/trigger_describe.py` renders a `trigger_json` predicate tree into plain English ("When the ticket is in Backlog AND ..."), and `on_success_json` into "Then move ticket to WIP, and set status to ...". Used by the /workflows page to show what each rule fires on without reading raw JSON. Also exposes `predicate_rows()` / `effect_rows()` for the Edit panel's structured-display blocks.
...
- **`on_success` effects:** `move_section` / `move_to` (move target ticket), `set_status`, `set_priority`, `set_automation_mode` ('auto'|'manual'|'paused', optionally with `pause_reason`), `set_is_container` (0|1), `add_tags`, `remove_tags`, `accept_ticket: true` (calls `actions.accept_ticket`; refuses silently unless target is in `For Review` with status `done`), `set_readiness_content {flag, from: "stdout"|"<literal>"}`, `clear_readiness_flag: "<flag>"`, `set_summary_oneliner: true`, and `apply_to: "self"|"parent"` (default `self`; when `parent`, all the above target the parent ticket).
- **Visual rule editor (`/workflows`):** Apple-mail-style attribute → operation → value rows. UI walks `conditions.ui_catalog()` (also exposed at `GET /api/workflow/catalog`) which maps each underlying predicate kind / on_success effect to a unified attribute domain (Section / Status / Tags / Readiness / Priority / Automation / Container / Parent / Children / Run / etc.). Closed-loop principle: every filterable attribute should also be settable so a rule that mutates a filter attribute self-terminates. `POST /api/workflow/lint` returns `{status: ok|warn|manual|empty, shared, ...}` — surfaced as an inline advisory when an effect doesn't mutate any filter attribute.
...
- **Next migration must be #21.** Live sequence: `1–5` (core), `6` (`ticket_branches`), `7` (`ticket_tags`), `8` (Kitchen — automation_subjects, runs, activity_events), `9–17` (factory-talk primitives + interactive runs), `18` (`tickets.summary_oneliner` + `tickets.summary_hash`), `19` (backfill `ticket_created` activity events for tickets that predate provenance tracking; recovers seek origin from description's `Source:` prefix), `20` (`endpoints` table + `workflow_agents.endpoint_id`; data backfill from legacy command/args). The `_migrations` table is a set of versions, not a strict sequence — gaps are harmless — but never reuse a number. Always check `grep "_migrations.*VALUES" src/db.py | tail` before adding one rather than trusting prose.
...
- **One canonical DB writer at a time across machines.** `~/.claude/ticket-takeaway/tickets.db` is the single source of truth for tickets/runs/events; running `serve.py` on two machines that point at copies of it diverges them silently because the unison-based claude-sync uses `sqlite3.backup()` snapshots that aren't merge-aware. As of 2026-05-10 the canonical writer is **llm-node** (`tt.rhino-balance.ts.net` via Tailscale Serve, llm port 8788); WSL `serve.py` is shut down. To migrate the canonical writer, the safe sequence is: (1) `sqlite3.backup()` snapshot the source DB to /tmp, (2) `scp` to the destination's runtime path, (3) restart the destination's `serve.py` so its open file handle releases the old DB, (4) stop the source's `serve.py`. PRODUCT_BACKLOG.md is the git-merge-friendly text form of the tickets table for cases where two machines do diverge — `tickets-cli.py seed` reconstructs the DB from it.

- **Page renderers must use origin-relative API base URLs.** Every `_render_*_page()` in `serve.py` emits client-side JS that calls back into the API. If those pages bake `http://localhost:{port}/{pid}/api` (the obvious default), the JS breaks the moment the page is reached via Tailscale Serve, port forwarding, or any reverse proxy: the browser fetches `localhost:{port}` on the *client's* laptop, not the serve host. Always use `f"/{pid}/api"` so the browser resolves it against whatever hostname loaded the page. Same rule for any inline JS that builds API URLs (e.g. Kitchen's `apiBase` for the run-detail panel).
...
- **Ticket creation always emits `ticket_created` with an `origin` payload.** `actions.add_ticket` now centrally fires the event (default origin `human`/`agent` based on `ActorContext.actor_type`). Callers that want a richer payload — `seek` (origin=`seek` + source_type/file/line), `tickets-cli.seed_project` (origin=`seed`), `tickets-cli._ingest_markdown_changes` (origin=`markdown_edit`), and the journey-gap path in `serve.py` (origin=`journey_gap` + run/journey ids) — pass `emit_created_event=False` to `add_ticket` and emit their own. New origins must also get a render branch in both the JS `_eventSummary` (in serve.py's _render_ticket_page f-string) and the Python `_ticket_created_summary` helper, otherwise the Activity tab shows "origin: <name>" generically. Never let two paths emit the event for the same ticket — pick one or pass the suppress flag.

- **Inline `onclick="..."` inside JS string literals must use HTML entities for nested quotes.** Pattern that broke twice in `serve.py`'s activity-tab JS: a JS source line like `runLink = ' <a onclick="setItem(\'foo\',\'bar\')">...</a>';` renders to `runLink = ' <a onclick="setItem('foo','bar')">...</a>';` because Python's triple-quoted f-string converts `\'` to `'`. JS then sees the inner `'foo'` quote as the closer of the outer string and fails with `Unexpected identifier 'tp'` (or similar), killing the *entire* `<script>` block — not just the broken line. Use `&apos;` for inner single quotes and `&quot;` for inner double quotes; the browser HTML-decodes them inside the attribute value at parse time. Same trap fires for `data-propose=\'...\'` patterns. Do not rely on backslash escapes inside Python triple-quoted f-strings that emit JS string literals.
