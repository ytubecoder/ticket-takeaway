...
**Workflow Bounce** (I-19, extended by the endpoint abstraction in feat/model-endpoints branch): Multi-agent prompt routing system. Users define **agents** (name + system prompt + `endpoint_id`) and **endpoints** (runtime: command + args + capabilities). Workflows are ordered steps, each with an agent + optional step instructions. Applying a workflow to a ticket bounces its content through the agent sequence. Primary agent (step 1) mediates disagreements.
...
- **System rows (`system=1`) are read-only — definition lives in source.** Applies symmetrically to system workflows AND system agents. Editor renders inputs disabled with a banner pointing at `workflows_seed.py`. Server enforces: `PUT` with non-`enabled` fields and `DELETE` on system workflows return `403 system_workflow`; `PUT` and `DELETE` on system agents return `403 system_agent`. To customise a system rule: duplicate (system workflows only — `POST .../duplicate` creates an editable user copy), or edit `workflows_seed.py` and restart serve.py to re-seed. System and user workflows are otherwise **symmetric**: same dispatcher loop, same ticket pool, same trigger evaluator, same on_success effects. The only execution difference is `ORDER BY system DESC, id ASC` in the dispatcher's workflow query, which gives system rows precedence on identical-trigger overlaps (deterministic tie-breaker, not a privilege gate).
