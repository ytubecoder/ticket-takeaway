...
- **Next migration must be #20.** Live sequence: `1–5` (core), `6` (`ticket_branches`), `7` (`ticket_tags`), `8` (Kitchen), `9–17` (factory-talk + interactive runs), `18` (`tickets.summary_oneliner` + `summary_hash`), `19` (backfill `ticket_created` events). The `_migrations` table is a set of versions — gaps are harmless — but never reuse a number. Run `grep "_migrations.*VALUES" src/db.py | tail` before adding one; also `git fetch origin main` first, since in-flight branches can race for the same number.
