...
- **System workflows (`system=1`):** Seeded immutable defaults. Users can toggle Enabled, can duplicate (creates an editable user-owned `system=0` copy via `POST .../duplicate`), but cannot edit the body in place — `PUT` with non-`enabled` fields and `DELETE` return `403 system_workflow`. System and user workflows are otherwise **symmetric**: same dispatcher loop, same ticket pool, same trigger evaluator, same on_success effects. The only execution difference is `ORDER BY system DESC, id ASC` in the dispatcher's workflow query, which gives system rows precedence on identical-trigger overlaps (deterministic tie-breaker, not a privilege gate). To dogfood: duplicate a system row, edit, optionally tighten the trigger or disable the system original to avoid shadowing.
