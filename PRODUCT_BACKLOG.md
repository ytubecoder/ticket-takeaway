...
Verified: exit=0 commit=0f05741 at=2026-07-29T05:07:20 cmd=python3 -m pytest tests/test_tdd_*.py -q
    ........................................................................ [  7%]
    ........................................................................ [ 15%]
    ........................................................................ [ 23%]
    ........................................................................ [ 30%]
    ...
    907 passed, 29 skipped in 23.10s

### B-71: Follow the Action — global activity-follow mode for the kanban
Priority: high | Status: for-review
Tags: automation, ux
Spec: A:b-71-follow-the-action-global-activity-follow-mode-for-the-kanban
Verified: exit=0 commit=2d53e43 at=2026-07-31T06:03:46 cmd=python3 -m pytest tests/test_tdd_*.py -q
    ........................................................................ [  7%]
    ........................................................................ [ 15%]
    ........................................................................ [ 22%]
    ........................................................................ [ 30%]
...
### B-72: OpenSpec surfacing — Spec tab, derived spec status, trigger filter, backfill
Priority: medium | Status: for-review
Tags: automation, ux
...
### B-73: install.py: deploy ruff.toml with the installed copy
Priority: medium | Status: proposed
Tags: quality, tech-debt
The repo IS ruff-clean: 'uvx ruff check src/' prints 'All checks passed!' because ruff.toml pins the ruleset and every rule the global hook flags is already a deliberate, commented ignore (BLE001, S110, S112, DTZ003, DTZ005, SIM118, PLW1510, TRY004, PLC0206). The bug is that install.py does not copy ruff.toml into the installed location (~/.claude/ticket-takeaway/), so when a Claude session edits the installed copy the global turn-end hook runs ruff there, finds no config, falls back to ruff defaults, and reports 221 'findings' that are all settled policy — blocking the turn and inviting the session to 'fix' them. That invitation is dangerous: acting on it reintroduces the bugs the ignores prevent. Clearest case is SIM118 — 'k in row.keys()' is CORRECT on a sqlite3.Row because 'k in row' iterates VALUES, so a cleanup pass would silently break every membership test. DTZ003/DTZ005 are the same: the ignore comment records that a tz-aware migration changes the stored isoformat string, a behavior change rather than style. This is not hypothetical — it cost a session on 2026-08-12, which filed this ticket in its original wrong form ('make the codebase ruff check-clean, 221 findings') before finding ruff.toml. FIX: have install.py deploy ruff.toml alongside the .py files, then confirm 'uvx ruff check ~/.claude/ticket-takeaway/serve.py' passes. ALSO CONSIDER: a short README in the installed copy noting it is a build artifact of ~/projects/ticket-takeaway/src/ and that edits belong upstream — the same session patched the installed serve.py directly, where the next install would have silently reverted it.
