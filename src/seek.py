...
MD_SKIP_FILES = {"README.md", "PRODUCT_BACKLOG.md", "PRODUCT_SPECIFICATION.md", "CHANGELOG.md"}
    ...
    try:
        result = subprocess.run(
            ["gh", "issue", "list", "--json", "number,title,body,labels",
             "--limit", "50", "--state", "open"],
            capture_output=True, timeout=10, cwd=project_path, text=True,
        )
...
def ingest(conn: sqlite3.Connection, project_id: str, items: list[DiscoveredItem]) -> list[str]:
