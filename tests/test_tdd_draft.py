...
class TestDraftColumn:
    ...
    def test_draft_default_is_zero(self, conn):
        conn.execute(
            "INSERT INTO tickets (id, project_id, title) VALUES ('T-01', 'test', 'Test')"
        )
    ...
    def test_draft_can_be_set_to_one(self, conn):
        conn.execute(
            "INSERT INTO tickets (id, project_id, title, draft) VALUES ('T-02', 'test', 'Draft', 1)"
        )
...
class TestSourceAttachmentIdColumn:
    ...
    def test_source_attachment_id_default_null(self, conn):
        conn.execute(
            "INSERT INTO tickets (id, project_id, title) VALUES ('T-03', 'test', 'Test')"
        )
        row = conn.execute(
            "SELECT source_attachment_id FROM tickets WHERE id = 'T-03'"
        ).fetchone()
...
class TestAttachmentsTable:
    ...
    def test_insert_attachment(self, conn):
        ...
        row = conn.execute(
            "SELECT * FROM ticket_attachments WHERE ticket_id = 'T-01'"
        ).fetchone()
    ...
    def test_same_session_different_tickets(self, conn):
        conn.execute(
            "INSERT INTO tickets (id, project_id, title) VALUES ('T-01', 'test', 'A')"
        )
        conn.execute(
            "INSERT INTO tickets (id, project_id, title) VALUES ('T-02', 'test', 'B')"
        )
        conn.execute(
            "INSERT INTO ticket_attachments (ticket_id, project_id, attachment_type, name) "
            "VALUES ('T-01', 'test', 'feedbacks_session', 'sess-1')"
        ...
        count = conn.execute(
            "SELECT COUNT(*) as c FROM ticket_attachments WHERE name = 'sess-1'"
        ).fetchone()
        assert count["c"] == 2
...
class TestSettingsTable:
    ...
    def test_insert_and_read_setting(self, conn):
        conn.execute(
            "INSERT INTO settings (key, value) VALUES ('feedbacks.enabled', 'true')"
        )
        row = conn.execute(
            "SELECT value FROM settings WHERE key = 'feedbacks.enabled'"
        ).fetchone()
    ...
    def test_upsert_setting(self, conn):
        ...
        conn.execute(
            "INSERT OR REPLACE INTO settings (key, value) VALUES ('foo', 'baz')"
        )
        row = conn.execute("SELECT value FROM settings WHERE key = 'foo'").fetchone()
        assert row["value"] == "baz"


from actions import add_ticket
...
class TestAddTicketDraft:
    ...
    def test_add_ticket_with_source_attachment_id(self, conn):
        conn.execute(
            "INSERT INTO tickets (id, project_id, title) VALUES ('P-01', 'test', 'Parent')"
        )
        conn.execute(
        ...
        row = conn.execute(
            "SELECT source_attachment_id FROM tickets WHERE id = ?", (tid,)
        ).fetchone()
...
class TestConfirmTicket:
    def test_confirm_draft_ticket(self, conn):
        ...
        assert (
            conn.execute("SELECT draft FROM tickets WHERE id = ?", (tid,)).fetchone()[
                "draft"
            ]
            == 1
        )
