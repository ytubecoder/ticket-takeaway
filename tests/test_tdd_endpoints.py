...
def test_build_invocation_no_placeholder_positional(endpoint_cli_codex):
    ...
    ep = Endpoint(id="x", name="x", endpoint_type="cli", command="codex", args=[])
...
def test_build_invocation_with_session_id_replaces_args(endpoint_cli_codex):
    ...
    argv = build_invocation(endpoint_cli_codex, "anything", session_id="abc-123")
    # resume_args fully replaces args
    assert argv == ["codex", "exec", "resume", "abc-123"]


def test_build_invocation_with_session_id_substitutes_both_placeholders(
    endpoint_cli_claude,
):
    from endpoints import build_invocation

    argv = build_invocation(endpoint_cli_claude, "continue", session_id="sess-xyz")
    assert argv == [
        "claude",
        "-p",
        "continue",
...
def test_build_invocation_session_id_without_resume_args_falls_through(caplog):
    ...
    ep = Endpoint(
        ...
        capabilities={"sessions": True},
        session_config={},
    )  # advertises sessions but no template
    argv = build_invocation(ep, "hi", session_id="s-1")
    assert argv == ["claude", "hi"]
    assert any(
        "advertises sessions but has no resume_args" in r.message
        for r in caplog.records
    )


def test_build_invocation_rejects_anthropic_api(endpoint_api_anthropic):
    from endpoints import UnsupportedEndpointType, build_invocation
...
def test_build_invocation_rejects_other_non_cli_types(etype):
    from endpoints import Endpoint, UnsupportedEndpointType, build_invocation
...
def test_build_invocation_rejects_invalid_args_not_array():
    from endpoints import Endpoint, EndpointMisconfigured, build_invocation
...
def test_extract_session_id_from_stderr(endpoint_cli_codex):
    ...
    sid = extract_session_id(
        endpoint_cli_codex, "", "Session: 019e1234-5678-90ab", started_before=0
    )
...
def db_pre_migration_20(tmp_path):
    ...
    # If _apply_migration_20 doesn't exist yet, the patch is harmless
...
def test_migration_20_idempotent(db_pre_migration_20):
    ...
    snapshot_a = conn.execute(
        "SELECT id, endpoint_id FROM workflow_agents ORDER BY id"
    ).fetchall()
...
def test_resolve_argv_raises_when_no_endpoint_and_no_command(caplog):
    ...
    agent = {"id": "agent_broken", "endpoint_id": None, "command": None, "args": "[]"}
    with pytest.raises(EndpointMisconfigured) as exc:
    ...
    assert any(
        "agent_broken" in r.message and r.levelname == "ERROR" for r in caplog.records
    )
