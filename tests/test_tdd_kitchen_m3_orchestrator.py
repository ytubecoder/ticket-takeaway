...
class _RecordingRunner:
    ...
    def __init__(
        self, mark_succeeded=True, hold_open_event: threading.Event | None = None
    ):
...
def _seed_eligible(env, ticket_id="B-1", project_id="p", priority="medium"):
    ...
    set_no_test_required(
        c, project_id, ticket_id, True, "docs only", ActorContext.human()
    )
    set_automation_mode(
        c, project_id, "ticket", ticket_id, "auto", ActorContext.human()
    )
    c.commit()
    c.close()
...
class TestEmptyState:
    ...
    def test_tick_with_ineligible_tickets_dispatches_nothing(self, env):
        ...
        c.execute(
            "INSERT INTO acceptance_criteria (ticket_id, project_id, text) VALUES ('B-1', 'p', 'X')"
        )
        c.commit()
        c.close()
        env["kitchen"].tick(env["conn_factory"], {"max_concurrent_runs": 3})
...
class TestDispatch:
    ...
    def test_eligible_ticket_dispatches_one_run(self, env):
        ...
        while time.monotonic() < deadline and (
            not runs or runs[0]["status"] in ("queued", "preparing", "running")
        ):
            time.sleep(0.05)
            runs = _runs(env)
        assert len(runs) == 1
    ...
    def test_priority_high_dispatched_first(self, env):
        ...
        while time.monotonic() < deadline and not rec.calls:
            time.sleep(0.05)
        assert rec.calls[0][2] == "B-high", (
            f"expected high-priority first; got {rec.calls}"
        )
...
class TestReconciliation:
    def test_stale_heartbeat_marks_run_stalled(self, env):
        ...
        c = env["conn_factory"]()
        row = c.execute(
            "SELECT status, error_class FROM runs WHERE id = ?", (rid,)
        ).fetchone()
        ev = c.execute(
