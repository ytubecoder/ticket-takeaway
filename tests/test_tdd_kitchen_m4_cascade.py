...
class TestCascade:
    ...
    def test_done_with_linked_journey_queues_one_cascade_run(
        self, conn, cascade_recorder
    ):
        from actions import ActorContext, move_ticket
    ...
    def test_multiple_linked_journeys_each_get_their_own_cascade(
        self, conn, cascade_recorder
    ):
        from actions import ActorContext, move_ticket

        _seed_ticket(conn)
        _seed_journey(conn, "J-1")
        _seed_journey(conn, "J-2")
        _link(conn, "J-1", "B-1")
        _link(conn, "J-2", "B-1")
        conn.commit()
        move_ticket(conn, "p", "B-1", "Done", actor=ActorContext.human())
    ...
    def test_cascade_fires_for_accept_ticket_too(self, conn, cascade_recorder):
        ...
        from actions import ActorContext, accept_ticket
    ...
    def test_cascade_does_not_break_when_kitchen_trigger_throws(
    ...
    def test_cascade_with_no_journey_tickets_table_is_safe(self, monkeypatch, tmp_path):
        ...
        c.row_factory = sqlite3.Row
        c.execute(
            "CREATE TABLE tickets (id TEXT, project_id TEXT, section TEXT, "
