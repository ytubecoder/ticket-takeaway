...
def _pass_verify(conn, tid="B-1", commit="abc1234"):
    rec = actions.VerifyRecord(
        command="tests/run-tests.sh", exit_code=0, commit=commit,
        ran_at="2026-07-23T02:00:00", output_tail="    OK",
    )
...
class TestSpecLinkFormat:
    @pytest.mark.parametrize("raw,lane,change,note", [
        ("A:b-44-knowledge-ingestion-pipeline", "A", "b-44-knowledge-ingestion-pipeline", ""),
        ("B:b-7-thing", "B", "b-7-thing", ""),
        ("C:none - rename only, no observable change", "C", "none", "rename only, no observable change"),
...
class TestVerifyRecordFormat:
    def test_round_trips(self):
        rec = actions.VerifyRecord(
            command="pnpm test && pnpm typecheck", exit_code=0, commit="deadbee",
            ran_at="2026-07-23T02:00:00", output_tail="    42 passed",
        )
        assert actions.parse_verify_record(rec.render()) == rec

    def test_failure_is_recorded_not_discarded(self):
        rec = actions.VerifyRecord(
            command="tests/run-tests.sh", exit_code=1, commit="deadbee",
            ran_at="2026-07-23T02:00:00", output_tail="    FAIL: test_x",
        )
    ...
    @pytest.mark.parametrize("raw", ["", "not a record", "exit=abc commit=x at=y cmd=z"])
...
class TestSpecLinked:
    ...
    def test_lane_a_and_b_require_a_change_name(self, conn, lane):
        t = _ticket(conn)
        actions.write_readiness_flag(conn, PROJECT, "B-1", "spec", f"{lane}:none - nope")
        ok, reasons = actions._spec_linked(conn, t)
    ...
    def test_lane_c_with_a_reason_passes(self, conn):
        ...
        actions.write_readiness_flag(
            conn, PROJECT, "B-1", "spec", "C:none - dependency bump, no behaviour change")
        ok, _ = actions._spec_linked(conn, t)
        assert ok

    def test_lane_a_with_a_change_passes(self, conn):
        t = _ticket(conn)
        actions.write_readiness_flag(conn, PROJECT, "B-1", "spec", "A:b-1-knowledge-ingestion-pipeline")
        ok, reasons = actions._spec_linked(conn, t)
...
class TestVerifyPassed:
    ...
    def test_failing_run_fails_and_quotes_the_output(self, conn, at_head):
        t = _ticket(conn)
        rec = actions.VerifyRecord(
            command="tests/run-tests.sh", exit_code=1, commit="abc1234",
            ran_at="now", output_tail="    FAIL: test_thing",
        )
...
class TestTestsCoveredNoLongerTraps:
    ...
    def test_the_legacy_no_test_required_path_still_works(self, conn, at_head):
        ...
        conn.execute(
            ...
            "WHERE id = 'B-1' AND project_id = ?", (PROJECT,))
        t = conn.execute("SELECT * FROM tickets WHERE id='B-1' AND project_id=?", (PROJECT,)).fetchone()
...
class TestAcceptGate:
    ...
    def test_lane_c_closes_through_the_same_gate_as_lane_a(self, conn, at_head):
        ...
        t = _ticket(conn)
        _pass_verify(conn)
        actions.write_readiness_flag(conn, PROJECT, "B-1", "spec", "C:none - dep bump only")
        gate = actions.evaluate_accept_gate(conn, t)
    ...
    def test_lane_a_needs_the_change_to_validate(self, conn, at_head, monkeypatch, tmp_path):
        t = _ticket(conn)
        _pass_verify(conn)
...
class TestGateIsSurfaceIndependent:
    ...
    def test_both_entrypoints_call_the_same_gate(self):
        ...
        serve_src = (Path(__file__).parent.parent / "src" / "serve.py").read_text(encoding="utf-8")
        ...
        cli_src = (Path(__file__).parent.parent / "src" / "tickets-cli.py").read_text(encoding="utf-8")
...
class TestForceOverride:
    def test_force_records_the_reason_rather_than_bypassing_silently(self, conn, at_head, tmp_path):
        _ticket(conn)
        actions.accept_ticket(
            conn, PROJECT, "B-1", str(tmp_path), "Proj",
    ...
    def test_force_notes_the_override_in_the_product_specification(self, conn, at_head, tmp_path):
        _ticket(conn)
        actions.accept_ticket(conn, PROJECT, "B-1", str(tmp_path), "Proj", force="deliberate")
    ...
    def test_without_force_a_failing_ticket_raises(self, conn, at_head, tmp_path):
        ...
        row = conn.execute("SELECT section FROM tickets WHERE id='B-1' AND project_id=?", (PROJECT,)).fetchone()
        assert row["section"] == "For Review"
        assert not (tmp_path / "PRODUCT_SPECIFICATION.md").exists()


class TestAcceptWritesEvidence:
    def test_passing_accept_records_the_verify_command_in_the_spec(self, conn, at_head, tmp_path):
        _ticket(conn)
        _pass_verify(conn)
        actions.write_readiness_flag(conn, PROJECT, "B-1", "spec", "C:none - dep bump only")
        actions.accept_ticket(conn, PROJECT, "B-1", str(tmp_path), "Proj")
        ...
        row = conn.execute("SELECT section, status FROM tickets WHERE id='B-1' AND project_id=?", (PROJECT,)).fetchone()
...
class TestResolveVerifyCommand:
    def test_workflow_toml_wins(self, tmp_path):
        ...
        (tmp_path / "tests" / "run-tests.sh").write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    def test_a_malformed_workflow_toml_does_not_disable_the_fallbacks(self, tmp_path):
        (tmp_path / "WORKFLOW.toml").write_text("this is not = valid toml [[[", encoding="utf-8")
        (tmp_path / "tests").mkdir()
        (tmp_path / "tests" / "run-tests.sh").write_text("#!/bin/sh\n", encoding="utf-8")
...
class TestRunVerifyRecordsRealOutput:
    def test_records_exit_code_and_output_for_a_failing_command(self, conn, tmp_path, monkeypatch):
        _ticket(conn)
        (tmp_path / "WORKFLOW.toml").write_text(
    ...
    def test_output_tail_is_indented_to_survive_markdown_regeneration(self, conn, tmp_path, monkeypatch):
        _ticket(conn)
        (tmp_path / "WORKFLOW.toml").write_text('[verify]\ncommand = "echo hello"\n', encoding="utf-8")
        monkeypatch.setattr(actions, "capture_commit_hash", lambda path: "cafe123")
        rec = actions.run_verify(conn, PROJECT, "B-1", str(tmp_path))
        assert rec.passed
